...
public class Archer : NetworkBehaviour
{
	...
	private void Shoot()
	{
		bow.GetComponent<Animator>().Play("Shoot", -1, 0f);
		isCoolDown = true;
		StartCoroutine(Reload());
	}

	private void SpawnProjectile(ulong clientId, Quaternion rot, Vector2 direction,int arrow, Vector3 pos)
	{
		GameObject go = Instantiate(arrowType[arrow], pos, Quaternion.identity);
	...
	private void AlignProjectile()
    {
		...
		Vector3 objectPos = Camera.main.WorldToScreenPoint(bow.transform.position);
		...
		ShootServerRPC(NetworkManager.Singleton.LocalClientId, rot, direction, currentArrow, bow.transform.position);
		SpawnProjectile(NetworkManager.Singleton.LocalClientId, rot, direction, currentArrow, bow.transform.position);
	}
	...
	{
		if(ability.abilityName == "Rapid Fire") 
		{ 
			currentCoolDownTime = GetComponent<PlayerStats>().FireRate.Value/ 3f;
			Shoot();
		}
